...
async function getLaunches(year: number) {
  try {
    ...
    for (let i = 0; i < data["data"]["launchesPast"].length; i++) {
      ...
      let incrementalYear = parseInt(
        data["data"]["launchesPast"][i].launch_date_utc.substring(0, 4)
      ); // Convert the data point UTC to a year
      let incrementalMonth = parseInt(
        data["data"]["launchesPast"][i].launch_date_utc.substring(5)
      ); // Convert the data point UTC to a month
      if (incrementalYear === year) {
        // If the data is within the year we want, then we should add it to the dictionary
        if (!(incrementalMonth in dict)) {
          // If the month is not in the dictionary yet, add the month as a key & set value of that key to 1 launch.
          dict[incrementalMonth] = 1;
        } else {
          // The month is in the dictionary, so increment the number of launches for that month
